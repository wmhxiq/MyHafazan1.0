...
export default function RekodHafazanList() {
  const [rekodList, setRekodList] = useState<RekodGroup[]>([]);
  const [loading, setLoading] = useState(true);
  //const [tarikhFilter, setTarikhFilter] = useState("");
  ...
  async function fetchRekod(idGuru: string) {
    ...
    const { data } = await supabase
      ...
      .not("HBmula", "is", null) // Only get records with Hafazan data
                  ...
                  >
                    ...
                    <td className="px-4 py-3 flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          router.push(
                            `/staf/rekod-hafazan/tambah?tarikh=${rekod.Tarikh}`,
                          )
                        }
